...
parser.add_argument('--use_sse', default=True, type=bool)
parser.add_argument('--sse_num', default=0.1, type=float)
parser.add_argument('--use_tcn', default=True, type=bool)
...
sampler = WarpSampler(user_train, usernum, itemnum, relation_matrix,
                      ...
                      n_workers=3, threshold_item = args.sse_num,use_sse =args.use_sse)
T = 0.0
t0 = time.time()
ff = open("use_sse:" + str(args.use_sse)+"|"+"sse_num:" + str(args.sse_num)+"|"+"use_tcn:" + str(args.use_tcn)+".txt",'w',encoding="utf-8")
try:
    for epoch in range(1, args.num_epochs + 1):
        ...
        if epoch % 5 == 0:
            ...
            ff.write('epoch:%d, time: %f(s), valid (NDCG@10: %.4f, HR@10: %.4f), test (NDCG@10: %.4f, HR@10: %.4f)' % (
            epoch, T, t_valid[0], t_valid[1], t_test[0], t_test[1])+'\n')
