...
class Model():
    def __init__(self, usernum, itemnum, timenum, args, reuse=None):
        ...
        with tf.variable_scope("SASRec", reuse=reuse):
            ...
            absolute_pos_K = embedding(  # 位置嵌入矩阵MPK表示自我注意机制中的键Key
            ...
            absolute_pos_V = embedding(  # 位置嵌入矩阵MPV表示自我注意机制中的值Value
                tf.tile(tf.expand_dims(tf.range(tf.shape(self.input_seq)[1]), 0), [tf.shape(self.input_seq)[0], 1]),
                vocab_size=args.maxlen,
                num_units=args.hidden_units,
                zero_pad=False,
            ...
            # Time Encoding
            time_matrix_emb_K = embedding(  # 相对时间间隔嵌入矩阵MRK表示自我关注中的键Key
                self.time_matrix,
            ...
            time_matrix_emb_V = embedding(  # 相对时间间隔嵌入矩阵MRV表示自我关注中的键Value
                self.time_matrix,
                vocab_size=args.time_span + 1,
                num_units=args.hidden_units,
                zero_pad=False,
            ...
            if args.use_tcn:
                # 在Q上增加TCN网络结构
                self.seq_q = tcn.TemporalConvNet(input_layer=self.seq, num_channels=[args.hidden_units],
                                                 kernel_size=2, dropout=0.01, init=False)
            else:
                self.seq_q = self.seq
            ...
            for i in range(args.num_blocks):
                with tf.variable_scope("num_blocks_%d" % i):
                    # Self-attention
                    self.seq = multihead_attention(queries=normalize(self.seq_q),
                                                   keys=self.seq_q,
