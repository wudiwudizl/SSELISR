...
def weightNormConvolution1d(x, num_filters, dilation_rate, filter_size=3, stride=[1],
    ...
    """
    name = get_name('weight_norm_conv1d_{}'.format(name), counters)
    with tf.variable_scope(name):
        if reuse:
...
def TemporalBlock(input_layer, out_channels, filter_size, stride, dilation_rate, counters,
                  dropout, init=False, atten=False, use_highway=False, gated=False,name =""):
...
def TemporalConvNet(input_layer, num_channels, kernel_size=2,
                    ...
                    atten=False,use_gated=False,name =""):
