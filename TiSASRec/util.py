...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, result_queue, SEED,threshold_item,use_sse):
    def sample(user):
        ...
        for i in reversed(user_train[user][:-1]):
            if use_sse and random.random() > threshold_item:
                i_tmp = np.random.randint(1, itemnum + 1)
                nxt_tmp = np.random.randint(1, itemnum + 1)
                seq[idx] = i_tmp
                time_seq[idx] = i_tmp
                pos[idx] = nxt_tmp
            seq[idx] = i[0]
...
class WarpSampler(object):
    def __init__(self, User, usernum, itemnum, relation_matrix, batch_size=64, maxlen=10,n_workers=1,threshold_item=0.1,use_sse=False):
